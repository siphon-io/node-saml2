...
AuthnRequestType.prototype.toXML = function toXML() {
  ...
  if (typeof this.ForceAuthn === "boolean") {
    root.setAttribute("ForceAuthn", this.ForceAuthn ? "true" : "false");
  }

  if (typeof this.IsPassive === "boolean") {
...
AuthnRequestType.prototype.fromXML = function fromXML(node) {
  RequestAbstractType.prototype.fromXML.call(this, node);

  if (node.hasAttribute("ForceAuthn")) {
    this.ForceAuthn = node.getAttribute("ForceAuthn").toLowerCase() === "true";
  }

  if (node.hasAttribute("IsPassive")) {
    this.IsPassive = node.getAttribute("IsPassive").toLowerCase() === "true";
  }

  if (node.hasAttribute("AssertionConsumerServiceIndex")) {
    this.AssertionConsumerServiceIndex = node.getAttribute("AssertionConsumerServiceIndex");
  }

  if (node.hasAttribute("AssertionConsumerServiceURL")) {
    this.AssertionConsumerServiceURL = node.getAttribute("AssertionConsumerServiceURL");
  }

  if (node.hasAttribute("ProtocolBinding")) {
    this.ProtocolBinding = node.getAttribute("ProtocolBinding");
  }

  if (node.hasAttribute("AttributeConsumingServiceIndex")) {
    this.AttributeConsumingServiceIndex = node.getAttribute("AttributeConsumingServiceIndex");
  }

  if (node.hasAttribute("ProviderName")) {
    this.ProviderName = node.getAttribute("ProviderName");
  }
