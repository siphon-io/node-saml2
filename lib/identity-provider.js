...
IdentityProvider.prototype.verify = function verify(doc, cb) {
  ...
  var signatureElement = xpath.select1("./*[namespace-uri()='urn:oasis:names:tc:SAML:2.0:assertion' and local-name()='Assertion']/*[namespace-uri()='http://www.w3.org/2000/09/xmldsig#' and local-name()='Signature']", doc);

  if (!signatureElement) {
    return cb(Error("no signature element found, but one was expected"));
  }
  ...
  dsig.verifySignature(signatureElement, options, cb);
};
